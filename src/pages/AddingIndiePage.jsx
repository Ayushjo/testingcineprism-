...
export default function AddIndiePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.streamingAt.trim()) {
      setSubmitStatus("error");
      setSubmitMessage("Please enter where the movie is streaming");
      setIsSubmitting(false);
      return;
    }
    ...
    try {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        setSubmitMessage("Indie movie added successfully!");
    ...
    <div className="min-h-screen bg-slate-950 p-6">
      <div className="max-w-4xl mx-auto">
        <motion.div
          ...
          className="mb-8"
        >
          <h1 className="text-4xl font-bold text-white mb-2">Add Indie Movie</h1>
          <p className="text-slate-400">
            Add a new movie to the indie collection
          </p>
        </motion.div>
        ...
        >
          ...
          <div className="space-y-2">
            ...
            <input
              ...
              onChange={(e) => handleInputChange("streamingAt", e.target.value)}
              className="w-full bg-slate-900/50 backdrop-blur-xl border border-slate-700 rounded-xl px-4 py-3 text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 transition-all duration-300"
              placeholder="e.g., Netflix, Mubi, Prime Video..."
              required
              disabled={isSubmitting}
            />
          </div>

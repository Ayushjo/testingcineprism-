...
import { X, Calendar, Film, ArrowLeft } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="p-4">
          <h3
            className={`mb-1 font-bold line-clamp-2 transition-colors ${
              theme === "light"
                ? "text-black group-hover:text-gray-700"
...
const IndianIndiePage = () => {
  ...
  useEffect(() => {
    const fetchIndianIndieMovies = async () => {
      setIsLoading(true);
      setError(null);
      ...
      try {
        ...
        const response = await axios.get(
          `https://api.thecineprism.com/api/v1/admin/fetch-byGenre/Sci-Fi`,
          {
        ...
        console.error("Error fetching Indian indie movies:", err);
  ...
  if (isLoading) {
    return (
      <div
        className={`flex min-h-screen items-center justify-center pt-20 transition-all duration-300 ${
    ...
    >
      ...
      <section className="relative py-12 sm:py-16">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <Link
            to="/indie"
            className={`inline-flex items-center gap-2 mb-8 px-4 py-2 rounded-xl border-2 transition-all duration-300 ${
              theme === "light"
                ? "border-gray-300 text-black hover:bg-gray-100 hover:border-black/40"
                : "border-white/10 text-white hover:bg-white/5 hover:border-white/20"
            }`}
          >
            <ArrowLeft className="w-4 h-4" />
            <span className="font-medium">Back to Indie Cinema</span>
          </Link>
          ...
          >
            <h1
              className={`mb-4 sm:mb-6 bg-clip-text text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black tracking-tight text-transparent transition-all duration-300 ${
                theme === "light"
                  ? "bg-gradient-to-r from-black via-gray-800 to-gray-600"
                  : "bg-gradient-to-r from-white via-emerald-200 to-slate-400"
              }`}
            >
              Top {movies.length} Indian Indie Films
            </h1>
            <p
            ...
            >
              Celebrating the vibrant voices of Indian independent cinema
            </p>
          </motion.div>
        </div>
      </section>
      ...
      {error && (
        <section className="relative pb-4">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div
              className={`mb-6 rounded-lg border p-4 transition-all duration-300 ${
                theme === "light"
...
export default IndianIndiePage;
